...
static volatile int16_t* audioBufferPtr = &audioBuffer[0];
...
{
	audioBufferPtr = &audioBuffer[BUFFER_SIZE];
	dataReadyFlag = 1;
}
...
{
	HAL_StatusTypeDef status = HAL_I2SEx_TransmitReceive_DMA(&hi2s2, (uint16_t*)audioBufferPtr, NULL, BUFFER_SIZE);
}
