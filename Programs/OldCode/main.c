...
void GetNextBuffer(void)
{
    ...
    do {
        const uint16_t nextCopyIndex = audioBufferIndex;
        for (uint16_t i = currCopyIndex; i < nextCopyIndex; ++i)
        {
            currAudioBuffer[i] = nextAudioBuffer[i];
        }
        currCopyIndex = nextCopyIndex;
    } while (audioBufferIndex < BUFFER_SIZE || currCopyIndex < audioBufferIndex);
}
...
void ConfigureTimer(void)
{
    ...
    htim6.Instance = TIM2;
    htim6.Init.Prescaler = 4799; // Prescaler to divide 216 MHz to 45 kHz
    htim6.Init.CounterMode = TIM_COUNTERMODE_UP;
    htim6.Init.Period = 999;     // Period to generate interrupt every 22.7 microseconds for 44.1 kHz
